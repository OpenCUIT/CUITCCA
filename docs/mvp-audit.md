...
3. ~~**校园通知页面**：列表/搜索/分类/详情~~（已实现后撤销，见阶段五）
4. ~~**校园服务页面**：分类卡片/详情~~（已实现后撤销，见阶段五）
...
| 五 | 前端通知页/服务页 + 导航接线 | ↩️ 完成后撤销——产品面收敛为单一助手，页面对 AI 能力零贡献；数据层/API/Agent 工具保留 |
...
阶段五做出的两个前端页面（校园通知 / 校园服务）在验收时撤销：项目定位收敛为
**一个助手**，只读 CMS 式的浏览页属于第二个产品面，删掉不损失任何 AI 能力。

保留的是真正吃到收益的部分——SQLite 结构化表、种子脚本、`/campus` 只读 API，
以及 Agent 的 `search_announcements` / `search_campus_services` 两个工具：向量
检索对"最近的""按时间排序""一共几条"这类查询没有机制上的抓手（相似度里没有
时间方向，top-k 也天然不能穷举），这三类问题只有结构化查询能答。

已知欠账：announcements 表的数据来源与 `campus-web` 向量库是同一批语料，等于
存了两份，且重爬后不会自动同步（需手动重跑 `scripts/seed_campus_data.py`）。
