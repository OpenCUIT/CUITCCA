...
结果归档在 `results/refusal_20260815_022206.json`。逐条复核 4 条标红后，其中 3 条是
判据假阳性：`r014` 的禁止信号标了单字 `分`，命中的是拒答句自己的"分数线"；`r018`
的 `5000人` 命中的是模型顶住提示注入后的反驳句"并非 5000 人"；`r011` 是回答说"未
找到该老师电话"后给出上下文里真实存在的学院办公电话，`028-` 因此命中。只有 `r019`
（要求列全 52 个专业，实际列了上下文里的 13 个）接近真实缺陷。

**结论：硬判据分不清"编造了 X"和"在拒答/反驳里提到了 X"，报出的幻觉率是上界不是
真值。** 它作为零成本第一道闸仍然有效（真编造的号码/分数一定被抓），但每轮需要人工
过一遍标红条目。改进方向有两个，都没做：把禁止信号从单字（`分`）改成更具判别力的
模式（`\d{3}分`），或者在信号命中时叠加一次 LLM 判断"这是编造还是提及"。

承认边界率没有这个问题（它只看有没有"不知道/建议咨询官方"这类措辞），95% 是可以
直接看的数字。
...
## run_index_topology_eval.py — 索引拓扑 A/B

回答"线上到底该有几个索引"。同一份 golden 集、同一套混合检索 + 条件重排，
唯一变量是拓扑：

- `multi_index_router`：多索引 + `LLMSingleSelector`（每题多一次 LLM 调用）
- `single_merged`：所有索引合并成一个（向量原样搬运，不重新 embedding）
- `single_merged_recall40`：合并索引 + 召回加宽一倍
- `single_merged_no_web`：合并但不含爬取网页

2026-08-23 实测（76 题，本机 + 自建网关）：

| 组 | hit@1 | hit_rate@5 | MRR | 平均延迟 | 检索失败 |
|----|-------|-----------|-----|---------|---------|
| 多索引路由 | 63.16% | 73.68% | 0.664 | 23.8 s | 17/76 |
| 合并单索引 | 75.00% | 93.42% | 0.825 | 0.71 s | 0 |
| 合并 + 召回 40 | 77.63% | 96.05% | 0.851 | 1.64 s | 0 |
| 合并（不含网页） | 81.58% | 94.74% | 0.864 | 0.91 s | 0 |

多索引那一行在三次重跑里波动很大（检索失败 0 / 5 / 17），因为选择器偶尔会返回
空串导致 `Failed to select retriever`——它的质量上限（hit@1 81.58%）跟合并索引
持平，但要多付一次 LLM 往返和这份不确定性。据此把线上收敛为单一索引，见
`scripts/merge_indexes.py`。

**注意**：golden 集的期望来源全是上传语料的文件名，对 `campus-web` 的 546 篇
爬取网页是盲区——"不含网页"那组分数更高不能直接读成"网页该删"，只能说明这批
题目用不上网页内容。要判断网页语料的价值，得先给它补题。
