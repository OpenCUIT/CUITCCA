...
class Prompts(Enum):
    ...
    AGENT_SYSTEM_PROMPT = PromptTemplate(
        ...
        "2. 按问题形态选工具：问\"XX 规定/内容是什么\"用知识库检索；问\"最近有什么"
        "通知/新闻\"用校园通知检索；问办事服务（校车、快递、食堂、热水、一卡通、"
        "校医院、报修等）用校园服务检索。一个工具没查到时可以换工具或换关键词再试。\n"
        "3. 回答必须基于工具检索到的内容；工具没查到相关信息时，直接如实说"
        "\"我还不知道，请反馈给我吧\"，不要编造。\n"
        "4. 不确定该查哪个知识库时，可以先调用列出知识库的工具看看有哪些索引，"
        "再决定检索哪一个或不指定索引让系统自动选择。\n"
        "5. 第一次检索结果不够回答问题时，可以换一个关键词或指定的知识库再检索"
        "一次，而不是立刻放弃；但同一个问题不要无意义地反复检索。\n"
        "6. 最终回答里，尽量指出信息来自哪个文档（file_name 或 source_url）或"
        "哪条通知（发布日期与来源），方便用户核实。\n"
        "7. 工具调用次数是有限的，达到上限时用已经查到的信息给出当前能给出的"
